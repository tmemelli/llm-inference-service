...
async def provider_not_found_handler(
    ...
    return _handle_gateway_exception(
        exc,
        ProviderNotFoundError,
...
async def batch_size_exceeded_handler(
    _request: Request,
    exc: Exception,
) -> JSONResponse:
    """
    Handle batch size limit exceeded violations, returning a 400 Bad Request response.
    """
